...
#        return super().web_search_read(domain, specification, offset=offset, limit=limit, order=order, count_limit=count_limit)
 ...
 #       self.env.company.sudo().must_create_orders_json = {}
    ...
    def import_sale_order(self, instance_id, is_force_update = False, from_method=None):
        ...
        for order in orders:
            ...
            # self.env.company.must_create_orders
            #################################################
        # if orders and self.env.company.must_create_orders_json:
        #     must_create_orders_json = dict(self.env.company.must_create_orders_json)
        #     [must_create_orders_json.pop(f'update_{ord["id"]}') for ord in orders if f'update_{ord["id"]}' in must_create_orders_json]
        #     _logger.error('must_create_orders_json ====')
        #     _logger.error(must_create_orders_json)
        #     self.env.company.sudo().update({'must_create_orders_json': must_create_orders_json})
        #     _logger.error(self.env.company.must_create_orders_json)

        #if orders and self.env.company.must_create_orders and from_method != 'cron':
        #    must_create_orders_json = dict(
        #        self.env.company.must_create_orders_json) if self.env.company.must_create_orders_json else {}

         #   [must_create_orders_json.pop(
          #      f'update_{ord["id"]}') for ord in orders if f'update_{ord["id"]}' in must_create_orders_json]

           # self.env.company.sudo().must_create_orders_json = must_create_orders_json
                ...
                if sale_order.state != 'done' or True:
                   ...
                   # _logger.error([int(woo_so_line_id) for woo_so_line_id in sale_order.order_line.mapped('woocomm_so_line_id') if int(woo_so_line_id) in lines_ids])
                    for sol_item in order['line_items']:
                        ...
                        self.env.cr.commit()
                    # woo_sale_rmv_lines_updated = [int(woo_so_line_id) for woo_so_line_id in sale_order.order_line.mapped('woocomm_so_line_id') if int(woo_so_line_id) not in lines_ids]
                    # _logger.error(f'woo_sale_rmv_lines_updated  {woo_sale_rmv_lines_updated}')
                    # if sale_order.state == 'sale':
                    #     sale_order.state = 'draft'
                    #     ord_line = self.env['sale.order.line'].sudo().search(
                    #         [('woocomm_so_line_id', 'in', woo_sale_rmv_lines_updated)])
                    #     if ord_line.exists():
                    #         # _logger.error(ord_line.read())
                    #         # _logger.error(ord_line.exists())
                    #         # _logger.error(ord_line)
                    #         # ord_line.sudo().unlink()
                    #         ord_line.product_uom_qty = 0
                    #         ord_line.price_unit = 0
                    #     # _logger.error([(6, 0, self.env['sale.order.line'].sudo().search([('woocomm_so_line_id', 'in', woo_sale_rmv_lines_updated)]).ids)])
                    #     # sale_order.sudo().order_line = [(6, 0, )]
                    ...
                    if order['status'] == "cancelled":
                        ...
                        soc.action_cancel()                         
        #if from_method != 'cron':
        #    must_create_orders_json = dict(self.env.company.must_create_orders_json) if self.env.company.must_create_orders_json else {}
        #    must_create_orders_json.pop(f'update_{order["id"]}') if f'update_{order["id"]}' in must_create_orders_json else None
        #    self.env.company.sudo().must_create_orders_json = must_create_orders_json
        return
    ...
    def woocomm_order_update_button(self):
        ...
        # line_items = [
        #     {'id': 45, 'name': 'WC ODOO VARS - S, black', 'product_id': 94, 'variation_id': 96, 'quantity': 1,
        #      'tax_class': '',
        #      'subtotal': '1.00', 'subtotal_tax': '0.00', 'total': '1.00', 'total_tax': '0.00', 'taxes': [],
        #      'meta_data': [{'id': 491, 'key': 'pa_size', 'value': 's', 'display_key': 'SIZE', 'display_value': 'S'},
        #                    {'id': 492, 'key': 'pa_coloor', 'value': 'black', 'display_key': 'COLOOR',
        #                     'display_value': 'black'},
        #                    {'id': 635, 'key': '_reduced_stock', 'value': '1', 'display_key': '_reduced_stock',
        #                     'display_value': '1'}],
        #      'sku': '', 'price': 1, 'image': {'id': 0, 'src': ''},
        #      'parent_name': 'WC ODOO VARS'}]
        # woocomm_so_line_id
        # get_order = woo_api.get("orders/%s"%self.wooc_id)
        # _logger.error('get_order /// get_order /// get_order /// get_order /// ')
        # _logger.error(get_order)
        # _logger.error(get_order.json())
        headers = {
...
#        _logger.error(f'values ==> {values}')
        #if len(self)>1:
        #    raise UserError('You cant modify many records once in woocommerce')
        if values.get('woocomm_status', False) or values.get('woocomm_order_note', False):
#            self.woocomm_order_update_button()
...
class SaleOrderLine(models.Model):
    ...
    #     _logger.error(inspect.stack()[1].function)
    #
    #     if inspect.stack()[1].function == 'write_real':
    #         if self.woocomm_so_line_id and self.order_wooc_id:
    #             data, woo_api = {}, self.order_id.init_wc_api(1)
    #             url = f"orders/{self.order_wooc_id}"
    #             wooc_order = woo_api.get(url).json()
    #             line_items_data = wooc_order.get('line_items')
    #             data = {"line_items": []}
    #             for line_item in line_items_data:
    #                 if line_item.get('line_items', False) and line_item['id'] == int(self.woocomm_so_line_id):
    ...
    #
    #             put_wooc_order = woo_api.put(f'orders/{self.order_wooc_id}', data)
    #             if put_wooc_order.status_code != 200:
    #                 raise UserError('Cant fetch data with woocommerce')
    #             elif put_wooc_order.status_code == 200:
    #                 _logger.error(self.env.company.must_create_orders)
    #                 self.env.company.sudo().update({
    #                     'must_create_orders': [int(ord_updated) for ord_updated in self.env.company.must_create_orders if
    #                                            ord_updated and ord_updated != '' and ord_updated != self.order_wooc_id]})
    ...
    def unlink(self):
        ...
        for rec in self:
            if rec.woocomm_so_line_id and rec.order_wooc_id:
                ...
                woo_api = rec.order_id.init_wc_api(1)
...
#                        })
    ...
    def unlink(self):
        ...
        for rec in self:
            if rec.woocomm_so_line_id and rec.order_wooc_id:
                data, woo_api = {}, rec.order_id.init_wc_api(1)
                url = f"orders/{rec.order_wooc_id}"
                wooc_order = woo_api.get(url).json()
                line_items_data = wooc_order.get('line_items')
                data = {"line_items": []}
                ...
                _logger.error(f'line_items_data =====>  {line_items_data}')

                data_with_headers = {**data, "headers": headers}
                _logger.error(rec.woocomm_so_line_id)
                put_wooc_order = woo_api.put(f'orders/{rec.order_wooc_id}', data_with_headers)
#                _logger.error()

                _logger.error(put_wooc_order.json())

                if put_wooc_order.status_code != 200:
                    raise UserError('Cant delete line data from woocommerce')
                elif put_wooc_order.status_code == 200:
                    order = put_wooc_order.json()
                    rec.order_id.update({
                        'woocomm_order_subtotal': float(order['total']),
                    ...
                    #must_create_orders = ' ,'.join([int(ord_updated) for ord_updated in rec.env.company.sudo().must_create_orders.split(',') if ord_updated and ord_updated != '' and int(ord_updated) != int(rec.order_wooc_id)])
#                    self.env.company.sudo().update({'must_create_orders': must_create_orders})
        return super().unlink()
    '''
    def _check_line_unlink(self):
        """ Check whether given lines can be deleted or not.
...
class SaleOrder(models.Model):
    ...
    @api.model
    def web_search_read(self, domain, specification, offset=0, limit=None, order=None, count_limit=None):
        if self.env.company.sudo().must_create_orders:
            self.env['sale.order'].sudo().import_sale_order(self.env['woocommerce.instance'].
                sudo().search([], limit=1, order='id desc'), is_force_update=True)
        return super().web_search_read(domain, specification, offset=offset, limit=limit, order=order, count_limit=count_limit)
    ...
    def get_all_orders(self, wooc_instance, limit=100):
        woo_api = self.init_wc_api(wooc_instance)
        must_create_orders = [str(r) for r in self.env.company.sudo().must_create_orders.split(',')]
        _logger.error(must_create_orders)
        url = f"orders" + f'?include={",".join(order_create for order_create in must_create_orders)}' if [r for r in must_create_orders if r and r != ''] else 'orders'
        _logger.error('//////////////////////////ddd')
        _logger.error(url)
        get_next_page = True
        page = 1
        while get_next_page:
            try:
                params = {'orderby': 'id', 'order': 'asc', 'per_page': limit, 'page': page} if not must_create_orders else {}
                # _logger.info('\n\n\n\n  page  =  %s \n\n\n\n' % (page) )
                orders = woo_api.get(url, params=params)
                _logger.error(f'orders ====??>   {orders}')
                page += 1
    ...
    def import_sale_order(self, instance_id, is_force_update = False):
        orders = self.get_all_orders(instance_id)
        ors = []
        for order in orders:
            ors.append(order)
            if not is_force_update:
            ...
            # self.env.company.must_create_orders
        if orders and self.env.company.must_create_orders:
            _logger.error(self.env.company.must_create_orders.split(','))
            must_create_orders_ids = [int(ord_str) for ord_str in self.env.company.must_create_orders.split(',') if ord_str and not self.search([('wooc_id', '=', int(ord_str))])]
            if must_create_orders_ids:
                _logger.error(f'must_create_orders_ids //// ==> {must_create_orders_ids}')
                self.env.company.sudo().update({'must_create_orders': str(must_create_orders_ids).replace('[', '').replace(']', '')})
            else:
                self.env.company.sudo().update({'must_create_orders': ''})
        return
    ...
    def create_sale_order(self, order, instance_id):
        ...
        if res_partner:
            ...
            if not sale_order:
                ...
                _logger.error(sale_order)
                _logger.error(sale_order.state)
                if sale_order.state != 'done' or True:
                    ...
                    _logger.error([int(woo_so_line_id) for woo_so_line_id in sale_order.order_line.mapped('woocomm_so_line_id') if int(woo_so_line_id) in lines_ids])
                    for sol_item in order['line_items']:
                        _logger.error('sol_itemsol_itemsol_itemsol_item')
                        _logger.error(sol_item)
                        res_product = self.env['product.product'].sudo().search(
                            ['|', ('woocomm_variant_id', '=', sol_item.get('product_id')), ('woocomm_variant_id', '=', sol_item.get('variation_id'))],
                            limit=1)
...
class SaleOrderLine(models.Model):
    ...
    def write(self, values):
        rtn = super().write(values)
        if inspect.stack()[1].function == 'write_real' and self.woocomm_so_line_id and self.order_wooc_id:
            data, woo_api = {}, self.order_id.init_wc_api(1)
            url = f"orders/{self.order_wooc_id}"
            wooc_order = woo_api.get(url).json()
            line_items_data = wooc_order.get('line_items')
            ...
            if rec.woocomm_so_line_id and rec.order_wooc_id:
                ...
                for line_item in line_items_data:
                    if line_item['id'] == int(rec.woocomm_so_line_id):
                        ...
                        data["line_items"].append(line_item)
                put_wooc_order = woo_api.put(f'orders/{rec.order_wooc_id}', data)
                if put_wooc_order.status_code != 200:
                    raise UserError('Cant fetch data with woocommerce')
                elif put_wooc_order.status_code == 200:
                    order = put_wooc_order.json()
